      ...
      <form onSubmit={onSubmit} className="mb-8 space-y-4 bg-black/20 rounded-2xl p-6 border border-white/10">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/20 bg-black/40 px-4 py-3 text-white placeholder-white/40 outline-none transition-all focus:border-violet-400/60 focus:bg-black/60 focus:ring-2 focus:ring-violet-400/20"
        ...
        <div>
          ...
          <textarea
            ...
            className="min-h-[140px] w-full resize-none rounded-xl border border-white/20 bg-black/40 px-4 py-3 text-white placeholder-white/40 outline-none transition-all focus:border-violet-400/60 focus:bg-black/60 focus:ring-2 focus:ring-violet-400/20"
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full sm:w-auto justify-self-start rounded-xl bg-gradient-to-r from-violet-400 via-violet-500 to-violet-600 px-8 py-3 text-sm font-semibold uppercase tracking-widest text-black shadow-[0_25px_60px_-20px_rgba(139,92,246,0.6)] transition-all hover:scale-105 hover:shadow-[0_25px_60px_-15px_rgba(139,92,246,0.8)] disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
        >
...
const CommentItem: React.FC<{
  ...
  return (
    <li className="group">
      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-black/40 to-black/20 p-5 sm:p-6 transition-all hover:border-violet-400/30 hover:shadow-lg">
        {/* Comment Header */}
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-400/20 to-violet-600/20 flex items-center justify-center text-violet-400 font-bold text-lg border border-violet-400/30">
              {comment.authorName.charAt(0)}
            </div>
            <div>
              <div className="text-sm font-semibold text-violet-400">{comment.authorName}</div>
          ...
          <button
            ...
            className="text-xs text-white/60 hover:text-violet-400 transition-colors px-3 py-1.5 rounded-lg hover:bg-white/10"
          >
            {isReplying ? '✕ لغو' : '↩ پاسخ'}
          </button>
        </div>
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2">
              <input
                ...
                className="w-full rounded-lg border border-white/20 bg-black/40 px-3 py-2 text-sm text-white placeholder-white/40 outline-none focus:border-violet-400/60"
                placeholder="نام شما"
                required
                maxLength={80}
              />
            ...
            <textarea
              ...
              className="w-full min-h-[80px] resize-none rounded-lg border border-white/20 bg-black/40 px-3 py-2 text-sm text-white placeholder-white/40 outline-none focus:border-violet-400/60"
            ...
            <div className="flex gap-2">
              <button
                type="submit"
                disabled={submitting}
                className="px-4 py-2 bg-gradient-to-r from-violet-400 to-violet-600 text-black text-xs font-semibold rounded-lg hover:scale-105 transition-all disabled:opacity-50"
              >
        ...
        {comment.replies && comment.replies.length > 0 && (
          <div className="mt-4 pr-4 sm:pr-8 border-r-2 border-violet-400/20">
